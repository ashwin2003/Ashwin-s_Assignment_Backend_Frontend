...
const Hero = () => {
  ...
  const getCases = async () => {
    ...
    try {
      const { data } = await axios.get(
        "https://assignment-backend-case.herokuapp.com/cases",
        config
      );
      console.log(data);
      setCases(data);
  ...
  const handleNewCase = async (e) => {
    ...
    try {
      const data = await axios.post(
        "https://assignment-backend-case.herokuapp.com/add",
        newCaseData
      );
      if (data) {
        setAddModal(false);
  ...
  const checkCaseNo = async (e) => {
    ...
    try {
      const { data } = await axios.get(
        `https://assignment-backend-case.herokuapp.com/cases?caseNo=${caseNo}`
      );
      if (data.length > 0) {
        setValidcaseNo(true);
        setOldcase(data[0]);
  ...
  const handleUpdateCase = async (e) => {
    ...
    try {
      const data = await axios.put(
        `https://assignment-backend-case.herokuapp.com/${updateCasedata.caseNo}`,
        updateCasedata
      );
      if (data) {
  ...
  const handleDeleteCase = async (e) => {
    ...
    try {
      await axios.delete(
        `https://assignment-backend-case.herokuapp.com/${caseNo}`
      );
  ...
  const handleFilter = async (e) => {
    ...
    queryParams +=
      fmanager.current?.value &&
      `&caseManager=${fmanager.current.value.trim()}`;
